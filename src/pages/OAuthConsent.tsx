...
export default function OAuthConsent() {
  ...
  const finishRedirect = useCallback((result: OAuthResult | null) => {
    ...
    if (!target)
      throw new Error("The authorization provider did not return a redirect.");
  ...
  useEffect(() => {
    ...
    void (async () => {
      ...
      if (!active) return;
      if (result.error) {
        setError("This connection request has expired or is invalid.");
        return;
      }
      const immediate = redirectFrom(result.data);
      if (immediate && !result.data?.client && !result.data?.client_name) {
    ...
    if (result.error) {
      setError(
        approved
          ...
          : "The connection could not be cancelled.",
      );
      setBusy(false);
      return;
    }
    ...
    <main className="min-h-screen bg-background px-4 py-12 text-foreground">
      <Card className="mx-auto max-w-lg border-border bg-card shadow-xl">
        <CardHeader className="space-y-4 text-center">
          ...
          <CardTitle>
            {details
              ? `Connect ${clientName} to ArtistrySynk`
              : error
          ...
          <CardDescription>
            {details
              ? `${clientName} will be able to use ArtistrySynk as you.`
              : error || "Please wait…"}
          </CardDescription>
        </CardHeader>
          ...
          {details && (
            <>
              ...
              <div className="space-y-3">
                {scopes.map((scope) => (
                  <div key={scope} className="flex gap-3 text-sm">
              ...
              <p className="text-xs text-muted-foreground">
                This does not bypass ArtistrySynk permissions or privacy
                controls.
              </p>
            </>
          )}
