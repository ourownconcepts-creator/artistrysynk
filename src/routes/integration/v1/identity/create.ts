  ...
  server: {
    handlers: {
      POST: async ({ request }) => {
        ...
        let client;
        try {
          const { audit, createIntent, enforceRateLimit, requireClient } =
          ...
          if (!parsed.success)
            return apiError(
              ...
              400,
              "invalid_request",
              "The identity creation request is invalid",
            );
          ...
          await audit({
            ...
            client,
            externalSubject: parsed.data.external_subject,
            metadata: { intent_id: intent.id, reused: intent.reused },
          });
          const claimUrl = intent.code
            ? new URL(
                `/integration/v1/claim?code=${encodeURIComponent(intent.code)}`,
                request.url,
              ).toString()
            : null;
          return apiSuccess(
