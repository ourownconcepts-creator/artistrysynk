...
export const Route = createFileRoute("/integration/v1/docs")({
  server: {
    handlers: {
      GET: async ({ request }) =>
        apiSuccess(requestId(request), {
          title: "ArtistrySynk Integration API v1",
          contract: "INTEGRATION.md",
          authorization: {
            delegated:
              "OAuth 2.0 authorization code through managed ArtistrySynk authorization",
            confidential:
              "HTTP Basic with the assigned client ID and one-time-issued client secret",
          },
          endpoints: [
            {
              method: "GET",
            ...
            {
              method: "GET",
              path: "/integration/v1/profile/{identity_id}",
              scope: "profile:read",
            },
            {
              method: "POST",
